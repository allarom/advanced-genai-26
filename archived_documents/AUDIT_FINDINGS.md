...
### P0-1. Stale PDFs — you submit the PDF, and both are out of date — **DONE (rebuilt)**
- **STATUS: DONE** — both `report.pdf` and `baseline_repro_report.pdf` rebuilt via `Rscript scripts/build_reports.R` (= the RStudio `rmarkdown::render` path, pdflatex). Built with no errors; visually verified (numbered sections incl. §7.1–7.9, figures with captions, tables wrap cleanly, appendices A and B each on a new page, ASCII diagrams fit). Also fixed: `\newpage` before Appendix B; numbered §7 subsections for consistency. **Re-run the build after any further `report.md` edits.**
