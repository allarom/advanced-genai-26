...
```
Step_4.1_extra_challenges.ipynb          Step_4_Evaluation.ipynb
├─ MemoryStore (M1 cache)                ├─ Setup (mount Drive, clone repo)
├─ MemoryAugmentedRAG (wraps rag_system) ├─ Load (%run Step_4.1) ← imports everything
...
**Relationship:** Step 4 Evaluation calls `%run Step_4.1` in Cell 3 to import `rag_system`, `MemoryStore`, `MemoryAugmentedRAG`, etc. You only upload Step 4 Evaluation to Colab — Step 4.1 runs automatically.
...
Cell 3: %run Step_4.1_extra_challenges.ipynb
...
|------|---------|----------|
| `Step_4_Evaluation.ipynb` | Generated notebook — the thing you run | ❌ No — regenerate via script |
| `scripts/build_step4_eval_notebook.py` | Generator — all notebook content lives here | ✅ Yes |
| `scripts/test_step4_evaluation.py` | Validation — checks structure & logic | ✅ Yes (if adding tests) |
| `Step_4.1_extra_challenges.ipynb` | Extra Challenges — memory + HITL | ✅ Yes (if changing memory logic) |
